...
int main(int argc, char* argv[])
{
    ...
    if (argc < 8)
    {
	    std::cerr << "Usage: " <<  argv[0] << " input_dcm_directory output_dir raw1_output_dir raw2_output_dir sigma concave_height [# threads]" << std::endl;
        return -1;
    }	

    int sigma = atoi(argv[5]);
    int concave_height = atoi(argv[6]);
    ...
    ConcaveFilter->SetHeight(concave_height);
    ...
    int thres = 0.5 * findThreshold3D(EightBitToFloatFilter->GetOutput(), LoGFilter->GetOutput());
