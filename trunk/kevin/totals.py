...
linepat = re.compile("[A-Za-z]: [0-9]+")

f = open(sys.argv[1], 'r')
lines = []
for line in f:
    if not linepat.match(line.rstrip()): continue
    lines.append(int(line.rstrip().split(" ")[1]))

sens = [lines[i] for i in range(1, len(lines), 4)]
tp = [lines[i] for i in range(2, len(lines), 4)]
fp = [lines[i] for i in range(3, len(lines), 4)]

print "Total sensitivity: %s" (np.mean(sens))
print "Total TPs: %s" % (len(tp))
print "Total FPs: %s" % (len(fp))
